      ...
      "VAttribute[directive=true][key.name='model']" (node) {
        ...
        if (name === 'input' && utils.hasAttribute(element, 'type', 'file')) {
          context.report({
            node,
            loc: node.loc,
            message: "'v-model' 指令不支持 'file' 输入类型."
          })
        }
        ...
        if (node.value) {
          if (!isLhs(node.value.expression)) {
            context.report({
              node,
              loc: node.loc,
              message: "'v-model' 指令需要有效的属性值作为 LHS."
            })
          }
          ...
          for (const reference of node.value.references) {
            ...
            if (variable != null) {
              context.report({
                node,
                loc: node.loc,
                message: "'v-model' 指令不能更新迭代变量 '{{varName}}' 本身.",
